...
```sh
dotnet add package FutureFlags.Client
```
...
```csharp
builder.Services.AddFutureFlags(builder.Configuration.GetSection("FutureFlags"));
```

```csharp
public sealed class CheckoutService(IFutureFlagsClient flags)
{
...
FutureFlags server independently. If you want a freshly started instance to answer correctly from
the moment it starts, or want an outage survived for longer than one process happens to stay up,
add [`FutureFlags.Client.Redis`](https://www.nuget.org/packages/FutureFlags.Client.Redis) — an
