...
/// <summary>
/// A consumer is allowed to put their own <see cref="IFutureFlagsClient"/> in — a stub in an
...
public class SubstitutedClientTests
{
    ...
    private sealed class StubClient : IFutureFlagsClient
    {
    ...
    private sealed class FailingClient : IFutureFlagsClient
    {
        public Task<bool> IsEnabledAsync(string key, CancellationToken cancellationToken = default) =>
    ...
    public async Task AReplacedClient_ShouldNotStopTheHostFromStarting()
    {
        ...
        using var host = BuildHost(services => services.AddSingleton<IFutureFlagsClient>(stub));
        ...
        Assert.Same(stub, host.Services.GetRequiredService<IFutureFlagsClient>());
    }
    ...
    {
        using var host = BuildHost(services => services.AddSingleton<IFutureFlagsClient, FailingClient>());
    ...
    public async Task AFailingRefresh_ShouldStopTheHostWhenAskedTo()
    {
        using var host = BuildHost(
            services => services.AddSingleton<IFutureFlagsClient, FailingClient>(),
            throwOnStartupFailure: true);

        await Assert.ThrowsAsync<FutureFlagsException>(
